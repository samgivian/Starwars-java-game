...
public class HothChar extends JPanel {
	public Timer t;
	public Timer t1;
	public Timer t2;
	...
	public HothChar() {
		...
		ImageIcon icon = new ImageIcon("hothwallpaper.png");
		...
		for (int i = 0; i < 22; i++) {
			ImageIcon imageIcon2 = new ImageIcon(new ImageIcon("h" + (int) (Math.random() * 3 + 1) + ".png")
					...
					try {
						...
						for (int o = 0; o < Blastlist.blasts.size(); o++) {
							...
							for (int q = 0; q < Droids.blasts.size(); q++) {
								...
								if (h.getX() <= s.getX() + 5 && h.getX() >= s.getX() - 5) {
									score += 1;
